...
(function(global, factory) {
  ...
  /**
   * 看法：
   * 这个文件主要处理的逻辑是，重写了数组原型对象上的方法，主要的方法是会操作数组本身的方法，比如
   * push 堆栈  返回数组长度
   * pop  弹栈  返回弹出项
   * shift 数组首项弹栈   返回弹出栈
   * unshift 数组首项堆栈   返回数组的长度
   * splice c r u d
   * sort 排序
   * reverse 倒序
   * 属于增强型重写，当去操作数组的一些方法，会触发一些其他的事件
   */
    ...
    for (i = 0; i < children.length; i++) {
      ...
      if (Array.isArray(c)) {
        ...
        if (c.length > 0) {
          ...
          // 如果第一个元素是文本节点  且 新数组 res 最后一个元素 为文本节点
